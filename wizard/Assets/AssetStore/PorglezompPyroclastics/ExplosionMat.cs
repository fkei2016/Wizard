...
public class ExplosionMat : MonoBehaviour {
	...
	void Start () {
        this.GetComponent<Renderer>().material = new Material(ExplosionMaterial);
	...
	void Update () {
		if (doUpdate) {
			Material rsm = this.GetComponent<Renderer>().sharedMaterial;
	...
	void SetShaderKeywords () {
        ...
        this.GetComponent<Renderer>().sharedMaterial.shaderKeywords = newKeywords.ToArray();
	}
}
